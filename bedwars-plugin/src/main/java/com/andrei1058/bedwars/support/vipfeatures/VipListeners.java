...
public class VipListeners implements Listener {
    ...
    public void onBockChange(BlockChangeEvent e) {
        if (BedWars.getAPI().getArenaUtil().getArenaByName(e.getLocation().getWorld().getName()) != null) {
            ...
            if (a.isTeamBed(e.getLocation())) {
                e.setCancelled(true);
                return;
            }
            Bukkit.getScheduler().runTask(BedWars.plugin, () -> {
                if (!e.isCancelled()
                        && BedWars.getAPI().getArenaUtil().getArenaByIdentifier(e.getLocation().getWorld().getName()) == a
                        && e.getLocation().getBlock().getType() != Material.AIR) {
                    a.addPlacedBlock(e.getLocation().getBlock());
                }
            });
        }
    }
