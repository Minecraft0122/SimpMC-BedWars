...
public final class ChineseConfigDocumentation {
    ...
    public static void main(ConfigManager config) {
        ...
        comment(config, ConfigPath.SB_CONFIG_SIDEBAR_USE_LOBBY_SIDEBAR, "计分板、TAB 玩家列表、页首页尾、血量显示及刷新周期设置。", "刷新周期单位为 tick，20 tick 约等于 1 秒。");
        comment(config, ConfigPath.SB_CONFIG_TAB_HEADER, "TAB 页首；每个列表元素显示为一行，支持 & 颜色代码和 {serverIp} 占位符。");
        comment(config, ConfigPath.SB_CONFIG_TAB_FOOTER, "TAB 页尾；每个列表元素显示为一行，支持 & 颜色代码和 {serverIp} 占位符。");
    ...
    public static void arena(ConfigManager config) {
        ...
        comment(config, ConfigPath.ARENA_SHOP_PROTECTION, "商店 NPC 周围保护半径。");
        comment(config, ConfigPath.ARENA_UPGRADES_PROTECTION, "升级 NPC 周围保护半径。");
        ...
        comment(config, ConfigPath.ARENA_GAME_RULES, "进入竞技场时应用的游戏规则，格式为 规则:值。", "默认阻止昼夜、天气、火焰和生物自然生成。");
    ...
    public static void shop(ConfigManager config) {
        ...
        comment(config, ConfigPath.SHOP_PATH_CATEGORY_ARMOR, "永久护甲分类；weight 用于阻止购买更低等级护甲。");
        ...
        comment(config, ConfigPath.SHOP_PATH_CATEGORY_UTILITY, "实用道具分类，包括 TNT、火球、珍珠等。");
    }
