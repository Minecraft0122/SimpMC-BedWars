...
项目从现在起按语义化版本决定版本号，不再把所有更新限制在 `2.10.x`：

- `PATCH`（例如 `2.10.54 -> 2.10.55`）：兼容的错误修复、性能优化、测试或文档完善；
- `MINOR`（例如 `2.10.x -> 2.11.0`）：向后兼容的新功能、新配置项或新 API；
- `MAJOR`（例如 `2.x -> 3.0.0`）：不兼容的 API、配置格式或游戏行为调整。
