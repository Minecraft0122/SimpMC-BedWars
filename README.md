...
- 游戏内 TAB 显示队伍名称前缀并把玩家名染成队伍颜色；玩家头顶名牌同步使用同一队伍颜色。
- 提供开局前真实队伍选择、固定队友邀请与自动均衡分队；每张竞技场可设置每队最少人数，并保证正常开局至少存在两支达标队伍。
...
- 火球普通初速度约为 1.6 格/tick；按住潜行键投掷时提高到约 2.4 格/tick，在开阔且区块已加载的区域可快速覆盖约 100–200 格。
- 支持掉线后 30 秒内重连；超时后按离开处理。
- 自动跟踪玩家放置的方块，并在每局结束后完整恢复竞技场世界。
- 竞技场与设置世界始终保持在 1000 tick 的白天，且禁止昼夜循环和跳时事件。
