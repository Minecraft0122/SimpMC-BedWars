...
- 先确认插件版本至少为 4.0.2，并在替换 JAR 后完整重启；不要使用 `/reload`。
- 游戏内 TAB 与头顶名牌由每个查看者的 scoreboard team 共同渲染；队伍数据包含前缀、队伍色玩家名、后缀和名牌可见性。关闭完整 TAB 前后缀不会关闭核心队伍色。
- 进入服务器、跨世界加入竞技场或离场后，插件会等待 Paper 确认客户端完成世界加载，再重放完整 scoreboard。若刚进服仍长期显示原版 TAB，请先确认服务端实际加载的是 4.0.2 JAR，而不是只替换了未重启的文件。
- 若只有头顶颜色失效，检查是否有其他计分板、Nametag 或大厅插件在 BedWars 之后替换玩家的 scoreboard；若只有 TAB 失效，检查其他插件是否持续改写 `playerListName`。
