...
- 先确认插件版本至少为 4.0.5，并在替换 JAR 后完整重启；不要使用 `/reload`。
...
- 进入服务器时，插件会等待新玩家的 `ADD_PLAYER` 条目已经广播后再更新已有观察者；首次世界加载时若大厅 TAB 尚未建立，也会立即补建。跨世界、加入竞技场、离场及 `showPlayer` 重建条目后均会重放对应状态。若刚进服仍长期显示原版 TAB，请先确认服务端实际加载的是 4.0.5 JAR，而不是只替换了未重启的文件。
