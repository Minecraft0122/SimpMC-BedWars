...
- 先确认插件版本至少为 4.0.3，并在替换 JAR 后完整重启；不要使用 `/reload`。
- 游戏内 TAB 与头顶名牌使用两条独立链路：TAB 直接向每个查看者发送“前缀 + 明确队伍色玩家名 + 后缀”的 PlayerInfo 显示名，头顶名牌由该查看者的 scoreboard team 负责。关闭完整 TAB 前后缀不会关闭核心队伍色。
- 进入服务器时，插件会等待新玩家的 `ADD_PLAYER` 条目已经广播后再更新已有观察者；首次世界加载时若大厅 TAB 尚未建立，也会立即补建。跨世界、加入竞技场、离场及 `showPlayer` 重建条目后均会重放对应状态。若刚进服仍长期显示原版 TAB，请先确认服务端实际加载的是 4.0.3 JAR，而不是只替换了未重启的文件。
- 若只有头顶颜色失效，检查是否有其他计分板、Nametag 或大厅插件在 BedWars 之后替换玩家的 scoreboard；若只有 TAB 失效，检查启动日志是否出现“无法初始化 Paper 1.21.11 TAB 数据包支持”，以及其他插件是否持续发送 PlayerInfo 显示名更新。插件会在相关事件发生时即时修复，并按配置周期批量自愈后续覆盖。
