...
| `/bw highlight` | 无 | 开关当前对局的队友高光；没有队友时会提示。发光状态是实体全局状态，其他玩家也能看到 |
...
| `/rejoin` | `bw.rejoin`（兼容 `bw.command.rejoin`） | 在有效窗口内重连 |

`bw.player` 不再是普通玩家使用基础命令的前置条件。`bw.shout` 自 2.11.0 起默认授予所有玩家，权限插件仍可显式设为 `false`；`bw.rejoin`（或兼容节点 `bw.command.rejoin`）仍需单独授予。BUNGEE 大厅会在执行命令时检查该节点，然后才向原竞技场发起跨服重连。
