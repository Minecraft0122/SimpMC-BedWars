...
| `/bw history [玩家UUID或在线玩家名] [页码]` | 无 | 按编号倒序分页查看玩家参加过的对局，省略玩家时查询自己 |
| `/bw record [玩家UUID或在线玩家名]` | 无 | 查看玩家所有已完成对局累计的击杀、最终击杀、死亡、拆床和 K/D，省略玩家时查询自己 |
...
`match`、`history` 和 `record` 从持久化对局明细查询；离线玩家使用 UUID，在线玩家可直接使用名字。`history` 包含进行中与中止的对局，`record` 只累计 `FINISHED` 对局；K/D = 普通击杀数 ÷ 死亡数，死亡数为 0 时 K/D = 普通击杀数。旧版仅保存的总战绩不能反推每场对局明细，因此 `/bw stats` 与新对局累计统计可能不同。BUNGEE 大厅配置同一 MySQL 后可查询竞技场子服保存的记录。
