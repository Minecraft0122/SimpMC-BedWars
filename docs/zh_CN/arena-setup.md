...
高级和引导式快速设置都会显示此命令。自动创建或手动新增队伍不会再把管理员设置的 `maxInTeam` 覆盖成 1/2；若之后使用 `setType`，所选类型仍会写入其标准队伍容量，随后可再用本命令覆盖。
...
最少人数必须介于 `1` 和当前 `maxInTeam` 之间，Tab 补全会根据当前最大人数动态列出候选值，不再固定停留在 1/2。插件会根据在线人数选择能够全部达到下限的参赛队伍，并在正式开局前再次确认至少两支队伍达标。旧配置中的总人数节点 `minPlayers` 会自动删除。
...
- 商店和升级村民保存独立 yaw，设置时请大致正对希望村民面向的方向；插件会自动吸附到最近的 90 度倍数，pitch 固定为 0。
...
`setType` 会设置竞技场分组及对应的每队人数。最少开局人数、世界边界、虚空高度和保护半径可在保存后编辑 `Arenas/<世界名>.yml`。竞技场时间固定为白天 1000 tick，昼夜循环不能由竞技场配置重新开启。
