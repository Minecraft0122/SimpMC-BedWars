...
高级和引导式快速设置都会显示此命令。自动创建或手动新增队伍不会覆盖管理员设置；`setType` 会写入所选类型的标准队伍容量，随后仍可用本命令覆盖。执行 `setType` 或 `setMaxInTeam` 时，每队开局下限会在当前 setup 会话中先同步为容量，避免新地图意外以缺人队伍开局。

设置容量时，插件只在当前 setup 会话中把每队开局下限同步为容量。需要提前开局时可继续执行：

```text
/bw setMinInTeam <人数>
```

范围为 `1..maxInTeam`。已保存竞技场的 `minInTeam` 不会在载入或配置升级时被覆盖。例如设置 `maxInTeam: 2`、`minInTeam: 1` 后，两名玩家即可被分到两支 1 人队开始游戏；其余空队本局不启用。
...
`setType` 会设置竞技场分组及对应的每队容量，并在当前 setup 会话中把每队开局下限重置为相同值；需要较早开局时，再使用 `/bw setMinInTeam` 降低下限。世界边界、虚空高度和保护半径可在保存后编辑 `Arenas/<世界名>.yml`。竞技场初始化时执行原版 `/time set noon` 的等价 API 操作，固定为正午 6000 tick，关闭昼夜、天气和随机方块刻；后续时间、下雨与雷暴变化由事件阻止，不使用周期遍历。
...
保存时插件会再次自动检查所有队伍的床，但不会扫描钻石或绿宝石生成器。若提示找不到床，应移动出生点、调整 `island-radius`，或手动执行 `setBed`；中央生成器请使用 `addGenerator` 手动添加。
