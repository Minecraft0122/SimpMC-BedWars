...
范围为 `1..maxInTeam`。已保存竞技场的 `minInTeam` 不会在载入或配置升级时被覆盖。例如设置 `maxInTeam: 2`、`minInTeam: 1` 后，两名未组队玩家即可被分到两支 1 人队开始游戏；如果两人已经组成同一小队，则会等待对手而不会强拆小队。分队时优先启用能够全部达到 `minInTeam` 的最多队伍，其余空队本局不启用。
