...
范围为 `2..地图总容量`。例如设置 `minPlayers: 2`、`maxInTeam: 4` 后，两名未组队玩家可分到两支 1 人队开始游戏；如果两人组成同一小队，则会等待对手而不会强拆小队。满足开局条件后，插件优先填满少数队伍，例如 6 名单人玩家通常分为 `4+2`，而不是平均分散到全部地图队伍。等待人数达到地图容量且开局倒计时仍超过 5 秒时，会固定缩短到 5 秒。旧 `/bw setMinInTeam` 仅作为兼容别名保留，并会提示改用新命令。
