...
- 游戏进行时恢复原仓库的 `{teamColor}{teamName} ` 队伍前缀，并把“队伍前缀 + 队伍色玩家名 + 后缀”作为完整 TAB 名称发送；玩家名字本身和头顶名牌都明确使用所属队伍颜色。即使关闭对应状态的完整 TAB 格式，竞技场仍保留不带前后缀的队伍色玩家名和头顶名牌。队伍按红→黄→绿→深绿→青→蓝→粉排列，非光谱色白→灰→深灰置后。同队的存活与淘汰玩家始终连续，并在整组内按完整玩家名字典序排列；纯旁观者最后。最终淘汰会立即同步给当前竞技场的所有 TAB 查看者。旧 `teammate-color` 配置会自动删除；简体中文语言配置架构 14 只迁移此前内置的空前缀，其他自定义文本保持不变。
...
- `group`、`display-name`、`maxInTeam`、`minInTeam`。每张竞技场只能属于一个组；该值同时用于匹配、生成器、开局物品、升级菜单和计分板等组专属配置。2.13.x 的 `groups` 列表会保留第一项并自动迁移回单值。
